...
- **Share Disk:** מחליף את Target Disk Mode בארכיטקטורת Apple Silicon. מאפשר לשתף את כונן המק ברשת או בכבל פיזי באמצעות פרוטוקול SMB.
...
- **Recovery Lock:** פרופיל MDM המגדיר סיסמה (14 תווים) ברמת ה-Secure Enclave שחוסמת את עצם הכניסה למצב ההתאוששות (מחליף את סיסמת הקושחה ב-Intel).
