...
2. בשורת החיפוש בפינה הימנית העליונה, הקלידו את המילה `bluetoothd` (או `diskarbitrationd`) ולחצו Enter.
...
5. כעת כבו והדליקו את ה-Bluetooth מתוך מרכז הבקרה (Control Center) או חברו את כונן ה-USB שלכם. שימו לב כיצד הזרם מציג מיידית אך ורק את אירועי התהליך שנבחר!
...
**מה לומדים:** כעת נלמד לבנות שאילתות מורכבות יותר כדי לדמות חיפוש של רכיבי מערכת ספציפיים, ממש כפי שמנהל רשת מחפש אירועים חריגים, ולשמור אותן לשימוש עתידי.
...
**מה לומדים:** מהנדסי התמיכה של אפל (או צוות ה-Tier 3 בארגון שלכם) זקוקים לתמונת מצב מקיפה של המערכת מרגע הקריסה או התקלה. יש להפיק את דו"ח ה-Sysdiagnose העצום, המכיל מאות מגה-בייטים של נתוני אבחון.
...
4. עליכם להסכים לתנאי האזהרה של אפל (המסבירים שהדו"ח מכיל מידע אישי ורגיש) ולהזין את סיסמת מנהל המערכת שלכם.
...
**מה לומדים:** חדר הבריחה הארגוני! משתמש קצה טוען שהוא לא מקבל פרופילים ופקודות מה-MDM הארגוני. כאיש IT, אתם חושדים שה-Firewall הארגוני חוסם את התקשורת של המק לשרתי אפל (APNs בפורט 5223). עליכם לספק הוכחה חותכת מתוך הלוגים לצוות ה-Network כדי שישחררו את החסימה.
...
2. הקלידו בשורת החיפוש `apsd` (Apple Push Service Daemon) או `mdmclient` ושנו את הפילטר ל-**Process**.
3. כעת, הקלידו בשורת החיפוש מילות מפתח המסגירות כשלים ברשת, כגון `timeout`, `failed` או `trust` (לשגיאות הצפנה ותעודות), והשאירו את הפילטר על **Any**.
