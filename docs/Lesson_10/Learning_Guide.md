...
- **איטיות בסביבת רשת (DS_Store):** אם חיבור ה-SMB איטי מאוד בזמן ניווט בתיקיות גדולות בשרת Windows, הדבר נובע מניסיון המק ליצור קבצי `.DS_Store`. מנהלי רשת יכולים למנוע זאת עם הפקודה בטרמינל:
  `המונח defaults write com.apple.desktopservices DSDontWriteNetworkStores -bool TRUE`

### פקודות דיאגנוסטיקה ואבחון רשת
- `המונח smbutil statshares -a`: מציג את חיבורי ה-SMB הפעילים ואת רמת ההצפנה וגרסת הפרוטוקול (למשל SMB 3.1.1).
- `המונח mount_smbfs`: לעיגון (Mount) כונני SMB ישירות מהטרמינל.
- `המונח ping -c 5 [server]` או `netstat -an`: אבחון תעבורת רשת בסיסית.

## שירותי שיתוף (Sharing Services) וקישוריות

- **המונח AirDrop:** שיתוף קבצים מקומי ללא ראוטר, המשתמש ב-Bluetooth לגילוי ו-Wi-Fi Direct (פרוטוקול AWDL) להעברת נתונים מהירה. במקרה של בעיות גילוי, כיבוי והדלקת ה-Wi-Fi עוזרת לאפס את ממשק ה-`awdl0`.
- **המונח Screen Sharing:** שיתוף מסך המבוסס על מנגנון VNC עטוף באבטחה. **שימו לב:** מערכת ההפעלה (TCC) דורשת מתן הרשאת Screen Recording ליישום, אחרת תוצג שגיאה או מסך שחור.
- **המונח Universal Control:** עבודה חלקה עם מקלדת/עכבר אחד בין כמה מחשבים/אייפדים סמוכים מאותו Apple ID (באמצעות Wi-Fi ו-Bluetooth ושירות Rapportd).

### פקודות גילוי ושיתוף
- `המונח sharing -l`: מציג את השירותים ותיקיות השיתוף הזמינות דרך ממשק ה-CLI (מחליף ניווט ב-System Settings).
- `המונח dns-sd -B _smb._tcp`: חיפוש והאזנה לשרתי SMB המכריזים על עצמם ברשת המקומית בטכנולוגיית Bonjour / mDNS.

## מחשב Mac Sharing Mode
...
## המונח Enterprise Seasoning: Single Sign-On (SSO)

- **המונח Kerberos SSO Extension:** תוסף מובנה במערכת macOS המאפשר הזדהות פעם אחת בלבד (Passwordless) מול ה-Active Directory בעזרת TGT (Ticket-Granting Ticket).
