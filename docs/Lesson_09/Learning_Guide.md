...
> [!IMPORTANT]
> **Stealth Mode בסביבה ארגונית:** הפעילת Stealth Mode יכולה לשבש כלי ניטור רשת שבודק את ה-Mac באמצעות Ping. אם Ping לא עובד — זה לא אומר שהמק כבוי; וודאו תחילה שהמק מחובר על ידי Bonjour / `dns-sd -B`. בסביבות ארגוניות הפעילו דרך MDM.

> [!NOTE]
> **הערה היסטורית:** פקודת `ifconfig` נחשבת כיום למיושנת ברוב הפצות לינוקס (שהחליפו אותה ב-`ip`), אך ב-macOS היא נותרה נתמכת ויעילה לחלוטין לאבחון ממשקים ברמת הליבה.
