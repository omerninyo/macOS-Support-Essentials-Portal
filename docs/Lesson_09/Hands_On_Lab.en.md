...
In this lab, we will practice managing Network Configurations visually and practically. We will learn how to configure Network Locations and manage Service Order priorities from within System Settings. In the second part, we will use built-in graphical tools like Activity Monitor and Wireless Diagnostics to perform network diagnostics, and in the third part, we will examine the built-in Firewall and monitor its activity using the Console application.

## Prerequisites

*   Mac updated to macOS 26 (Tahoe).
*   User with Admin privileges.
*   Access to the System Settings interface.
*   Active network connection (Wi-Fi or cable).

---

## Exercise 1: Managing Network Locations and Service Order

In this scenario, we will simulate switching between an "Office" network configuration and a "Home" network configuration, changing DNS servers and interface settings via the user interface (GUI).
...
3.  In the menu that opens, select **Locations** and then **Edit Locations**.
...
3.  Click the three dots `...` menu again -> **Locations**, and verify that `Office-Lab` is now marked with a V as enabled. (Switching to the new location temporarily disconnects the network and applies clean settings for this profile).
...
### Step 4: Adjusting Service Order
...
3.  In the window that opens, drag one of the other interfaces (for example, Ethernet or your iPhone USB connection, if present) above the Wi-Fi connection.
...
As organizational support personnel, it is important to know which tools are available in the system for checking network integrity, without opening Terminal.
...
4.  At the bottom of the window, review the graph showing the system's overall network traffic (Packets in/out). This is an excellent tool for identifying applications that are "hogging" the network, such as a stuck cloud sync in the background.
...
3.  Review the information displayed. Here, all IP settings, Subnet Mask, and MAC addresses for all your interfaces in different locations are consolidated in one place.
...
2.  Notice the rich information revealed: IP address, Tx Rate, and signal strength and noise (RSSI and Noise).
3.  Click **Open Wireless Diagnostics...**.
4.  A network diagnostics wizard will open. You can click **Continue** and let the system analyze the Wi-Fi environment to detect stability issues, background noise from neighbors' routers, and other interferences.

---

## Exercise 3: Firewall and Monitoring Blocks in Console

In managed environments (MDM), the Application Layer Firewall is often enabled to protect organizational computers from hostile connections on public networks.
...
5.  Ensure that the "Automatically allow built-in software to receive incoming connections" setting is checked, click **OK** to save.
...
5.  Now, try opening a new application that requires network access, or ask a classmate to try sending you a ping or AirDrop. If there is a block (Drop) or the system grants the application temporary approval, you will see the messages pop up here in real-time. This tool is essential for troubleshooting issues where organizational software stops working due to blocked incoming communication.
...
1.  **Quickly change DNS for the Wi-Fi interface (`networksetup`):**
    ...
    ```
    *This command applies the exact same Google DNS servers that we manually configured earlier through the interface. To reset back to the local router's DNS, use the word `Empty` instead of the addresses.*
...
2.  **Check Firewall status behind the scenes (`socketfilterfw`):**
    ...
    ```
    *This is the CLI tool that manages the Application Firewall. The command prints back to the screen whether the firewall is enabled or disabled, without needing to open System Settings.*

<!-- src_hash: b47dac65a3c5dba04b7ce9dcf416bf0ad516bbc728c81a5de81bdc0335a837d1 -->
