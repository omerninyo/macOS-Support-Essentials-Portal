...
## Lab 1: Diagnosing Networks with Wireless Diagnostics
...
3.  **Monitoring Performance:**
    ...
    -   Wait about a minute and observe the changes in the graph. This tool is excellent for identifying momentary connection drops or spikes.
...
4.  **Scanning Networks and Channels:**
    ...
    -   This window will display all networks in the vicinity (sometimes including those not shown in the regular menu), which Channels they use, and the Noise level on each channel. This is an ideal tool for IT support needing to identify interference caused by overlapping channels.
    -   Close the Scan window.

5.  **Packet Sniffer – *Demonstration Only*:**
    -   From the `Window` menu, select `Sniffer`.
    -   Observe the option to select a network Channel and bandwidth. Clicking `Start` will disconnect you from the internet and begin capturing Packets floating in the air on that channel, saving them as a file for analysis. (For this exercise, there is no need to start capturing).
...
4.  **Exiting Safe Mode:**
    ...
    -   Upon startup, the system resumes its normal operation (disk integrity verification performed behind the scenes and cache clearing may sometimes resolve the issue "on their own" even after returning to normal mode).

---

## Lab 3: Investigating the System with System Information
...
1.  **Opening System Information:**
    ...
    -   Hold down the `Option` (⌥) key on the keyboard, and click the Apple () menu in the top-left corner of the screen (or top-right in a Hebrew system).
...
2.  **Extracting Hardware Information:**
    ...
    -   Review data such as the processor model (Chip), amount of Memory, and Serial Number.
...
## Lab 4: Supporting a Managed Environment (Enterprise Seasoning) - Profile Isolation in GUI

**Objective:** To isolate issues when a Mobile Device Management (MDM) server and enforced Configuration Profiles are involved, using the System Settings interface.
...
3.  **Attempting to Remove a Profile for Isolation:**
    ...
    -   *Note:* In most real-world managed environments, the profile will be set as Non-Removable and cannot be removed here; you will need to remove it via the MDM server to complete the isolation process. In this lab, we will suffice with familiarizing ourselves with the location and the attempt.

---

## Bonus Exercise for IT Professionals: Terminal

Although macOS GUI interfaces provide most of the information needed for diagnostics, in managing a fleet of computers (MDM) or working remotely, advanced IT professionals rely on command-line tools. Here are the equivalent Terminal methods for the operations we just performed:
...
<!-- src_hash: d39843ee918dc3677ca75fceb7f786a997719588f8205bfc4fed0c14232d9093 -->
