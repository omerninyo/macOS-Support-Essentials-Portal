...
* **המונח Isolation:** אסטרטגיית הליבה של Apple לפתרון תקלות, שמטרתה לחצות את הבעיה לשניים בכל שלב כדי לבודד את המקור:
...
* **חברת Apple Diagnostics:** כלי אבחון חומרה המובנה בקושחה (Firmware) שנועד לבדוק רכיבים פיזיים כגון זיכרון (RAM), מאווררים, סוללה וחיישנים. במחשבי Apple Silicon מפעילים אותו מתוך תפריט האפשרויות (לחיצה ארוכה על כפתור ההדלקה) ולחיצה על `Command + D`.
* **המונח Verbose Mode:** מצב הפעלה (רלוונטי יותר במחשבי Intel עם מקשי `Command + V`) המציג טקסט של תהליך האתחול במקום לוגו התפוח. כיום במחשבי Apple Silicon מוחלף בעיקר בבדיקת לוגים עמוקה לאחר עליית המערכת.
* **כלי Activity Monitor & Console (היסטוריה קצרה):** אפליקציית Activity Monitor החלה את דרכה ב-2003 (Mac OS X Panther) כאיחוד של הכלים Process Viewer ו-CPU Monitor. בעבר, ה-Console עבד מול קבצי טקסט פשוטים, אך החל מ-macOS Sierra ב-2016 המערכת עברה ל-Unified Logging System - מערכת לוגים בינארית ואחידה המציגה אלפי אירועים בשנייה.
...
!!! tip "Deep Dive 🤿: מה Activity Monitor באמת מודד?"
    ...
    * **המונח CPU %:** יכול לחצות את ה-100% (כל ליבה נספרת כ-100%), אך המדידה לא משקפת עומס מדויק בגלל הארכיטקטורה הא-סימטרית (E-cores ו-P-cores).
    * **המונח Energy Impact:** זהו ציון (Score) יחסי המבוסס על אלגוריתם, ולא נתון פיזי בוואט (Watts).
...
* **המונח App Memory Leaks (דליפות זיכרון באפליקציות):** מצב שבו אפליקציה צורכת עוד ועוד זיכרון ללא הרף ולא משחררת אותו חזרה למערכת. הדבר גורם לאיטיות רבה במק (תופעת ה-Beachball), אך לרוב ללא רעש מאווררים מוגבר (המאפיין עומס על המעבד). הפתרון הוא לעקוב אחרי צריכת הזיכרון בלשונית Memory ב-Activity Monitor, ולכבות את האפליקציה לחלוטין (Quit). לעומת זאת, דליפות בזיכרון הליבה (Kernel Memory Leaks) הן נדירות וחמורות בהרבה, ויובילו לקריסת מערכת מוחלטת (Kernel Panic). [לקריאה נוספת מהמאמר של Eclectic Light](https://eclecticlight.co/2026/06/19/what-can-you-do-when-an-app-uses-too-much-memory/)

## המונח Safe Mode

המונח Safe Mode הוא כלי אבחון עוצמתי שפותר לעיתים קרובות בעיות בעצם הפעלתו בלבד, כיוון שהוא מנקה ומאפס רכיבי מערכת.
...
* **חברת Apple Silicon:** כיבוי ה-Mac. לחיצה ממושכת על כפתור ההדלקה עד להופעת "Loading startup options". בחירת כונן ההפעלה הרצוי, לחיצה ארוכה על מקש `Shift`, ולחיצה על "Continue in Safe Mode".
* **מעבדי Intel:** הדלקת ה-Mac ולחיצה מיידית וממושכת על מקש `Shift` עד להופעת חלון ההתחברות.
...
* `המונח system_profiler SPSoftwareDataType` - מציג מידע על גרסת ה-macOS, מזהה ה-Build, גרסת הקרנל וזמן הפעולה (Uptime).
* `המונח system_profiler SPHardwareDataType` - מציג מידע על המעבד, מספר הליבות, זיכרון, מספר סידורי ו-UUID של החומרה.
* `המונח system_profiler SPPowerDataType` - מציג את נתוני הסוללה, כולל Cycle Count ומצב בריאות הסוללה (Condition).
* `המונח system_profiler > ~/Desktop/MacReport.txt` - מייצא את הדו"ח השלם והמלא לקובץ טקסט אל שולחן העבודה לצורך שליחה לתמיכה.
...
* `המונח networkQuality` - מריץ את הבדיקה הרגילה בחיבורים מקבילים מול שרתי Apple.
* `המונח networkQuality -v` - מריץ את הבדיקה עם פירוט נרחב (Verbose), כולל פירוט מדדי זמני השהייה (Latency) ספציפיים.
* `המונח networkQuality -s` - מריץ את בדיקת הרשת באופן טורי (Sequential) ולא במקביל, כלי נהדר לבודד בעיות בתעבורת הנתונים.
...
* `המונח log show --last 10m` - מציג את כל הלוגים שנרשמו ב-10 הדקות האחרונות.
* `המונח log show --predicate 'process == "kernel"' --last 1h` - מסנן ומציג רק לוגים ששייכים לתהליך הקרנל המרכזי מהשעה האחרונה.
* `המונח log show --info --debug --last 5m > ~/Desktop/logs.txt` - שואב גם לוגים זמניים בדרגות Info ו-Debug (שלא נשמרים תמיד בדיסק לאורך זמן) ומייצא אותם לקובץ לניתוח נוח.
...
* `המונח klist` - מציג את רשימת כרטיסי ה-Kerberos הפעילים של המשתמש. אם הרשימה ריקה, למשתמש אין הרשאות שקופות.
* `המונח klist -v` - מציג את כל המידע המורחב (Verbose) על הכרטיס, כולל תאריכי התחלה, תאריכי תפוגה וסוג ההצפנה, כדי לאתר תוקף שפג.

## פקודות תקשורת נוספות לאבחון ובידוד

* `המונח ping -c 4 8.8.8.8` - שליחת 4 חבילות מידע לשרתי Google לבדיקת חיבוריות בסיסית לאינטרנט וזמני תגובה.
* `המונח traceroute google.com` - מציג את נתיב ניתוב החבילות (Hops) מה-Mac ועד לשרת היעד, מאפשר לאתר היכן נתקעת התקשורת בתוך הרשת הארגונית.
* `המונח ifconfig` - מציג מידע טכני נמוך על כרטיסי הרשת הפיזיים (ה-MAC Addresses).
* `המונח networksetup -listallhardwareports` - הדרך הקלה והקריאה ביותר לזהות את כתובות ה-MAC הפיזיות לכל סוגי החיבורים במערכת.
